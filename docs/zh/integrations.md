...
如果您希望在 Claude Code 会议中直接让 Claude 运行 `tis` 命令，只需确保：

1.  **安装库**：在您的开发环境中运行 `pip install train-in-silence`。
2.  **配置环境变量**：Claude Code 会继承您当前的 Shell 环境变量。请确保设置了必要的 API Key：
    ```bash
    export VAST_API_KEY="您的_KEY"
    export RUNPOD_API_KEY="您的_KEY"
    ```
...
| 变量名 | 说明 |
| :--- | :--- |
| `VAST_API_KEY` | Vast.ai 的 API 密钥 |
| `RUNPOD_API_KEY` | RunPod 的 API 密钥 |
